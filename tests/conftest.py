...
flags:
    ...
    python -m pytest ./projects/des_y3/tests --high=1 --mask=ones
...
# the masks the comparison sweep can run under; "frozen" is the
# baseline mask of the frozen contract and "ones" keeps every point
# (no scale cuts). Any non-"frozen" name selects the frozen TATT
# dataset variant "<tatt_dataset stem>_<name>.dataset".
...
def pytest_addoption(parser):
    ...
    27-29) at the frozen default settings; --high=1 repeats them
    ...
    try:
        parser.addoption(
            ...
            help="the scale-cut mask of the CFASTPT-vs-FASTPT "
                 "comparison: frozen (the baseline contract mask, "
